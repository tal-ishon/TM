...
def load_glove_embeddings(file_path, embedding_dim):
    ...
    # add a random vec to represent unknown word
    embeddings.insert(0, get_random_norm_vec(len(embeddings[1])))
...
def get_final_embedding(word_to_ix, embed):
    ...
    for index, word in enumerate(vocabulary):
        ...
        """
        Use when don't want the same random vector to all words not in embedding
        """
        # if not ix: 
        #     counter += 1
            # words.append(word)
        #     corpus_embed[index] = get_embed_bigram(word, word_to_ix, embed)
        #     continue

        corpus_embed[index] = embed[ix]

        # Note: if ix is 0 - embed[ix] should return the random vector

    embed_list = list(corpus_embed.values())
    # print("# words not in embedding: {}".format(counter))
    # save_file_txt("words_not_in_embedding", words)
...
def main():
    ...
    if len(args) == 1:
        ...
        save(corpus_embed, "Results/WordEmbedding/embedding")
        save(corpus_to_ix, "Results/WordEmbedding/word_to_ix")
...
###
# validation
# word_to_ix = torch.load("Results/WordEmbedding/word_to_ix")
# Embedding = torch.load("Results/WordEmbedding/embedding")
