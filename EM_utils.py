...
def get_filtered_corpus(corpus):
    ...
    filtered_corpus = [
        ' '.join(sorted(set(dictionary[word_id] for word_id, freq in bow)))
        for bow in bow_corpus
    ]
...
def save_word_predictions(pred_path, word_to_ix_path, save_path):
    pred = torch.load(pred_path)
    word_to_ix = torch.load(word_to_ix_path)

    word_embed = {}
    for i, word in enumerate(word_to_ix.keys()):
        word_embed[word] = pred[i]

    torch.save(word_embed, save_path)
